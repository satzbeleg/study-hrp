...
TESTS = ['STS12', 'STS13', 'STS14', 'STS15', 'STS16']
...
for i, model in enumerate(MODELS):
    for j, numbool in enumerate(NFEATS):
        ...
        for seed in SEEDS:
            ...
            tmp = [dat[test]["all"]["spearman"]["wmean"] for test in TESTS]
...
# save tables
with open("table-sts.tex", "w") as fp:
...
fig.tight_layout()
fig.savefig("res-sts-memorate-averaged.png")
...
for k, testname in enumerate(TESTS):
    ...
    for i, modelname in enumerate(MODELNAME):
        modelname = f"{modelname} ({baselines1[i, k]:.3f})"
        x = [100 * compression_rate(numbool, MODELDIM[i]) for numbool in NFEATS]
        ax.fill_between(
    ...
    ax.xaxis.set_major_formatter(matplotlib.ticker.ScalarFormatter())
    fig.tight_layout()
    fig.savefig(f"res-sts-memorate-error-{testname}.png")
